...
export default function ClearingLedger() {
    ...
    async function fetchLedger(){
        if (navigator.onLine){
            return await axiosInstance.get(`apis/ClearingLedger/${fields.FromDate}/${fields.ToDate}`)
            .then(res=>{
                let data  = res;
                if (data['error'] === true){
                    ...
                    setRows(data);
                    setLoading(false);
                    localStorage.removeItem('ClearingLedger');
                }
            })
            .catch(error=>{
                setLoading(false);
            ...
            setRows(JSON.parse(localStorage.getItem('ClearingLedger')));
        }
    }
    ...
    return (
        ...
        >
           {/* Title */}
           <Grid item xs={12} >     
                <Typography variant="h4" gutterBottom  color='primary'>Clearing Ledger</Typography>
            </Grid>
