...
function DataTable(props) {
    ...
    await axiosInstance
      ...
      .then((res) => {
        if (res["error"] === true) {
          ...
          var d = res["data"]["data"];
          for (let order in d) {
            d[order].d_p = d[order].party.discount.discount;
  ...
  async function fecthReleventRecoveries(id) {
    await axiosInstance
      .get(`apis/GetRecoveriesByOrder/${id}`)
      .then((res) => {
        if (res["error"] === true) {
          toast.error(`Error Occures ${res["message"]}`);
        } else {
  ...
  async function DispatchUpdate() {
    await axiosInstance
      .put(`apis/DispatchTable/${selecteddispatch}/`, {
        ...dispatchFields,
        party_order: selecteddispatch,
  ...
  const OpenInvoice = (row) => {
    ...
    fecthReleventRecoveries(row.id);
            ...
            <div className="invoice-box" id="PDFinvoice">
              <table>
                <thead>
                  ...
                  <tr className="information" key={121}>
                    <td colSpan={4}>
                      <table>
                        <tr key={155}>
                          <td>
                  ...
                  <tr className="heading" key={19382}>
                    <td>Product</td>
                    <td>Qty</td>
                    <td>Rate</td>
                    <td>Total</td>
                ...
                <tfoot>
                  <tr className="item last" key={1523}>
                    <td>
                      <b>Total</b>
                    </td>
                    <td>
                      <b>{selectedPartyOrder.pdt_qty__sum}</b>
                    </td>
                    <td></td>
                    <td>
                      <b>{selectedPartyOrder.gross_total}</b>
                    </td>
                  </tr>
                  <tr className="total" key={324}>
                    <td></td>
                    <td colSpan={4}>Discount : {selectedPartyOrder.d_p} %</td>
                  </tr>

                  <tr className="total" key={232}>
                    <td></td>

                    <td colSpan={34}>
                      Discount Amount: {selectedPartyOrder.discounted_amount}
                    </td>
                  </tr>
                  <tr className="total" key={142}>
                    <td></td>

                    <td colSpan={4}>Freight: {selectedPartyOrder.freight}</td>
                  </tr>
                  <tr className="total" key={223}>
                    <td></td>

                    <td colSpan={4}>
                      Net Total: {selectedPartyOrder.total_amount}
                    </td>
                  </tr>
                </tfoot>
              </table>
              ...
              <Divider />
              <Typography color="secondary" varient="h6" component="h6">
                Recoverries{" "}
              </Typography>
              <TableContainer component={Paper}>
                <Table aria-label="recoveries table">
                  <TableHead>
                    <TableRow>
                      <TableCell align="center">Recovery by</TableCell>
                      <TableCell align="center">Recovery Date</TableCell>
                      <TableCell align="center">Recovered With</TableCell>
                      <TableCell align="center">Head</TableCell>
                      <TableCell align="center">Recovered Amount</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {!recoveries
