...
function Adjustments() {
    ...
    async function FetchSenderReleventHeader(type){
        return await axiosInstance.get(`apis/${type}/`)
        .then(res=>{
            let data  = res.data;
            if (data['error'] === true){
                ...
                for (let h in headers){
                    ...
                    if (type === 'Party'){
                        headers[h].discount = headers[h].discount.discount;
                    }
                }
    ...
    const LedgersTitle = ['Party','SalesOfficer','Bank','Incentive','Clearing','Sales','Freight','Discount','Cash']
    
    const SenderTypeChange = (e) =>{
        setSenderType(e.target.value);
        if (e.target.value === 'Party' || e.target.value === 'SalesOfficer' || e.target.value === 'Bank'){
    ...
    const reciverFieldsChange = (event)=>{
        try{
            if (reciverLgType === 'Bank' || reciverLgType === 'Party' || reciverLgType === 'SalesOfficer'){
                const index = event.target.selectedIndex;
                const optionElement = event.target.childNodes[index];
                const optionElementId = optionElement.getAttribute('id');
                const obj = JSON.parse(optionElementId);
            ...
            <Grid container justifyContent='center' alignItems="center" spacing={4} className={classes.root} component={Paper}>
                ...
                <Grid item xs={12} md={3}>
                    <Grid container direction='column' spacing={3}>
                        ...
                        <Grid item >    
                            <TextareaAutosize required value={fields.reciverDescription} onChange={reciverFieldsChange} name='reciverDescription' placeholder="Enter Discription" maxRows={12} className={classes.textArea}/>
                        </Grid>
                        <Grid item >
                            <InputField  size='small'
                                label="Amount" 
